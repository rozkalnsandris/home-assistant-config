...
`START`, `turpini`, or equivalent continuation may carry safe Git-only documentation, reviewed YAML/config source and tests from fresh canonical GitHub state through Ready in one coherent batch, including branch, PR, validation/review and up to two scope-preserving corrections. Batch 2-5 closely related same-risk items when coherent. Merge remains explicit.
