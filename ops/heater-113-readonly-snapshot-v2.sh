...
container_tmp="$(docker exec "$container" mktemp -d /tmp/ha-heater113v2-XXXXXXXX 2>/dev/null)" || blocked PRIVATE_TEMP_CREATE_FAILED
...
from tools.materialize_heater_retire_candidate_privately import extract_live_target, materialize_candidate_texts
...
try:
    ...
    c1, c2, _ = materialize_candidate_texts(
        (repo / "packages" / "silditajs.yaml").read_text(encoding="utf-8"),
        (repo / "packages" / "heater_scheduler.yaml").read_text(encoding="utf-8"),
        private_target,
    )
    matches = [
        hmac.compare_digest(live1.read_bytes(), c1.encode("utf-8")),
        hmac.compare_digest(live2.read_bytes(), c2.encode("utf-8")),
    ]
except Exception:
...
[ -s "$snapshot" ] || blocked SNAPSHOT_EMPTY
if ! python - "$snapshot" <<'PY' >/dev/null 2>&1
import json, sys
from pathlib import Path
...
try:
    ...
    snap = {"ok": False, "reason": "SNAPSHOT_INVALID_JSON"}
...
report = {
    "schema": 2,
    "decision": decision,
    "source_gate": {"sha": sha, "tree": tree, "parent": parent},
    "home_assistant": {
        "version": version,
        "running": True,
